...
export default function (root, {tools, ingredients}, {userId}) {
  ...
  const toolsId = map(toolsOnly, tool => tool._id) // Get only Id to do the search

  const ingredientsAll = map(ingredients, ingredient => Ingredients.findOne(ingredient))  // Get the elements like an array, if dont exist, put undefined
  const ingredientsOnly = ingredientsAll.filter(function (n) { return n !== undefined }) // Delete elements undefined in the array
  const ingredientsId = map(ingredientsOnly, ingredient => ingredient._id) // Get only Id to do the search
